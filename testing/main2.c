...
int compress(stream in, stream out) {
    ...
    // for (int kilo = 0;kilo< arraySize;kilo++){
    //     array[kilo] = in->get(in);
    // }
...
int decompress(stream in, stream out ) {
    ...
    while (true) {
        ...
        // fread(readBuffer, 1, 2, fileInput);
        size_t bytesRead = 0;
        ...
        if (bytesRead >= 2) {
            ...
            output(out,writeBuffer, (size_t) match);
...
int main() {
	...
	string_stream str_in = { sget, 0,
		"x:0.093999997	y:12	0.999899983	0.0037	-0.0003	0	-0.0018	-0.0024	4.488900185	-0.061000001	-0.061000001	0	0	0.0006	0.0073	0	0.0006	-0.0073	0.0006	0.0073	0.999899983	30.37709999 0.093999997	12	0.999899983	0.0037	-0.0003	0	-0.0018	-0.0024	4.488900185	-0.061000001	-0.061000001	0	0	0.0006	0.0073	0	0.0006	-0.0073	0.0006	0.0073	0.999899983	30.37709999", 0};
	string_stream str_out = { sget, sput, buf, 0 };
    string_stream str_out2 = { sget, sput, buf, 0 };
	file_stream file = { 0, file_put, stdout };

    int ln = compress((stream)&str_in, (stream)&str_out);


    fprintf(stdout, "\nInput file: %s",str_in.string);
	fprintf(stdout, "\nEncoded file: %s",str_out.string);
    
    int ln1 = decompress((stream)&str_out, (stream)&file);
    // fprintf(stdout, "\nOutput file: %s", file);
